...
SELECT m.name, brand_name, headquarters FROM models m JOIN brands b ON m.brand_name = b.name WHERE m.name = 'Mustang';
...
SELECT m.name, brand_name, founded FROM models m LEFT JOIN brands b ON (m.brand_name = b.name) WHERE m.year = 1960;
...
SELECT b.name, b.founded, m.name FROM brands b LEFT JOIN models m ON b.name = m.brand_name WHERE b.discontinued IS NULL;
...
SELECT m.name, m.brand_name, b.founded FROM models m INNER JOIN brands b ON b.name = m.brand_name;
...
SELECT b.name, founded FROM brands b LEFT JOIN models m ON b.name = m.brand_name WHERE m.name IS NULL;
...
SELECT b.name, m.name, m.year, b.discontinued, b.discontinued-m.year AS years_until_brand_discontinued FROM models m LEFT JOIN brands b ON m.brand_name = b.name WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
